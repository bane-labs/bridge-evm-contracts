...
library GasBridgeLib {
        ...
        returns (bytes32)
    {
        ...
        for (uint256 i = 0; i < depositsLength; i++) {
            ...
            bytes32 depositHash = _hashGasBrideOp(depositData.nonce, depositData.to, depositData.amount);
            parent = BridgeLib._computeNewRoot(parent, depositHash);
        }
        return parent;
    }

    function _hashGasBrideOp(uint256 _nonce, address _to, uint256 _amount) internal pure returns (bytes32) {
