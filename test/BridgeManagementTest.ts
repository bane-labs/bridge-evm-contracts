...
describe("Bridge Management", function () {
    ...
    describe("Deployment", function () {
        it("Should have the right relayer", async function () {
            ...
            expect(await bridgeManagementImpl.relayer()).to.equal(relayer.address);
        });
        ...
        it("Should have the right validators", async function () {
            ...
            expect(await bridgeManagementImpl.validators(0)).to.equal(validator1.address);
            expect(await bridgeManagementImpl.validators(1)).to.equal(validator2.address);
            expect(await bridgeManagementImpl.validators(2)).to.equal(validator3.address);
            expect(await bridgeManagementImpl.validators(3)).to.equal(validator4.address);
            expect(await bridgeManagementImpl.validators(4)).to.equal(validator5.address);
            expect(await bridgeManagementImpl.validators(5)).to.equal(validator6.address);
            expect(await bridgeManagementImpl.validators(6)).to.equal(validator7.address);
            await expect(bridgeManagementImpl.validators(7)).to.be.revertedWithoutReason();
        });
        ...
        it("Should have the right governor", async function () {
            ...
            expect(await bridgeManagementImpl.governor()).to.equal(governor.address);
        });
        ...
        it("Should have the right securityGuard", async function () {
            ...
            expect(await bridgeManagementImpl.securityGuard()).to.equal(securityGuard.address);
        });
    ...
    describe("Bridge role setting", function () {
        ...
        it("Set validators with unique address", async function () {
            ...
            await expect(await bridgeManagementImpl.validators(0)).to.be.equal(new_validators[0]);
            await expect(await bridgeManagementImpl.validators(1)).to.be.equal(new_validators[1]);
            await expect(await bridgeManagementImpl.validatorThreshold()).to.be.equal(2);
        });
        ...
        it("Set validators multiple times", async function () {
            ...
            await expect(tx).to.emit(bridgeManagementImpl, "SetValidators").withArgs(new_validators, 2);
            await expect(await bridgeManagementImpl.validators(0)).to.be.equal(validator1.address);
            await expect(await bridgeManagementImpl.validators(1)).to.be.equal(validator2.address);
            await expect(await bridgeManagementImpl.validators(2)).to.be.equal(validator3.address);
            await expect(await bridgeManagementImpl.validatorThreshold()).to.be.equal(2);
            ...
            await expect(await bridgeManagementImpl.validators(0)).to.be.equal(validator1.address);
            await expect(await bridgeManagementImpl.validators(1)).to.be.equal(validator2.address);
            await expect(await bridgeManagementImpl.validators(2)).to.be.equal(validator4.address);
            await expect(await bridgeManagementImpl.validatorThreshold()).to.be.equal(2);
        });
        ...
        it("Set relayer", async function () {
            ...
            await expect(await bridgeManagementImpl.relayer()).to.be.equal(validator2.address);
        });
        ...
        it("Set governor", async function () {
            ...
            await expect(await bridgeManagementImpl.governor()).to.be.equal(validator2.address);
        });
        ...
        it("Set securityGuard", async function () {
            ...
            await expect(await bridgeManagementImpl.securityGuard()).to.be.equal(validator2.address);
        });
